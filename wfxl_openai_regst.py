    ...
    if EMAIL_API_MODE == "outlook_file":
        ...
        with _OUTLOOK_INIT_LOCK:
            if not _OUTLOOK_INITIALIZED:
                used_emails = set()
                if os.path.exists(OUTLOOK_USED_LOG):
                    with open(OUTLOOK_USED_LOG, "r", encoding="utf-8") as f:
                        used_emails = {l.strip() for l in f if l.strip()}
                    ...
                    with open(OUTLOOK_FILE_PATH, "r", encoding="utf-8") as f:
                        for line in f:
                            ...
                            if line and "----" in line:
                                ...
                                email_clean = mail_str.strip()
                                if email_clean not in used_emails:
                                    _OUTLOOK_ACCOUNTS_QUEUE.put((email_clean, pwd_str.strip()))
                else:
        ...
        # 记录到已使用日志，防止重启重试
        with open(OUTLOOK_USED_LOG, "a", encoding="utf-8") as f:
            f.write(f"{acc_email}\n")
            
        print(f"[{ts()}] [INFO] 从文件获取 Outlook 账号: {acc_email} (已记入 {OUTLOOK_USED_LOG})")
